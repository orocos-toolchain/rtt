  ...
  {}
    ...
    DataSource<T>* DataSource<T>::narrow(DataSourceBase* dsb) {
        ...
        if ( dsb->hasServer() ) {
            ...
            return ORO_Corba::ExpressionProxy::CreateDataSource<T>( expr.in() );
        }
        // See if the DS contains an Any.
        DataSource<CORBA::Any_var>* aret = dynamic_cast< DataSource<CORBA::Any_var>* >( dsb );
        if (aret){
            return ORO_Corba::ExpressionProxy::CreateConstant<T>( aret->get().in() );
        }
#endif
        // all failed:
        return 0;
    ...
    AssignableDataSource<T>* AssignableDataSource<T>::narrow(DataSourceBase* dsb) {
        ...
        if ( dsb->hasServer() ) {
            ::Orocos::Expression_var expr = dsb->server() ;
            return ORO_Corba::ExpressionProxy::CreateAssignableDataSource<T>( expr.in() );
        }
#endif
        // all failed:
        return 0;
