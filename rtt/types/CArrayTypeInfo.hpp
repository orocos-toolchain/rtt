...
namespace RTT
{
    namespace types
    {
        ...
        class TemplateCArrayInfo: public TemplateTypeInfo<T, has_ostream>
        {
        public:
            TemplateCArrayInfo(std::string name) :
            ...
            virtual base::DataSourceBase::shared_ptr getPart(base::DataSourceBase::shared_ptr item,
                ...
                log(Error) << "TemplateCArrayInfo: No such part (or invalid index): " << id_name->get() << id_indx->get() << endlog();
                return base::DataSourceBase::shared_ptr();
            }
        };
    }
