...
{
  rtos_mutex_lock(&m); v->counter += i; rtos_mutex_unlock(&m);
}
...
{
  int ret = 0;
  rtos_mutex_lock(&m); v->counter -= i;  ret = (v->counter == 0); rtos_mutex_unlock(&m);
  return ret;
}
...
{
  rtos_mutex_lock(&m); ++(v->counter) ; rtos_mutex_unlock(&m);
}
...
{
  int ret = 0;
  rtos_mutex_lock(&m); --(v->counter);  ret = (v->counter == 0); rtos_mutex_unlock(&m);
  return ret;
}
